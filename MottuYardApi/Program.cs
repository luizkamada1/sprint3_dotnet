...
builder.Services.AddSwaggerGen(c =>
{
    c.SwaggerDoc("v1", new OpenApiInfo
    {
        ...
        Version = "v1",
        Description = "API de mapeamento de pátios da Mottu (pátios, zonas e motos), com boas práticas REST."
    });
...
app.MapGet("/api/patios", async (AppDbContext db, HttpContext http, [FromQuery] int page = 1, [FromQuery] int pageSize = 10) =>
{
    ...
    Hateoas.AddCollectionLinks(result, http, "/api/patios");
...
app.MapGet("/api/patios/{id:int}", async (int id, AppDbContext db, HttpContext http) =>
{
    ...
    Hateoas.AddSelf(res, http, $"/api/patios/{id}");
    Hateoas.AddLink(res, "zonas", http, $"/api/zonas/patio/{id}", "GET");
...
app.MapPost("/api/patios", async ([FromBody] PatioCreateDto body, AppDbContext db, HttpContext http) =>
{
    ...
    var dto = new PatioDto(entity.Id, entity.Nome, entity.Cidade, entity.Estado);
    var res = new Resource<PatioDto>(dto);
    Hateoas.AddSelf(res, http, $"/api/patios/{entity.Id}");
    return Results.Created($"/api/patios/{entity.Id}", res);
})
...
app.MapPut("/api/patios/{id:int}", async (int id, [FromBody] PatioUpdateDto body, AppDbContext db) =>
{
    var entity = await db.Patios.FindAsync(id);
    if (entity is null) return Results.NotFound();
...
app.MapDelete("/api/patios/{id:int}", async (int id, AppDbContext db) =>
{
    var entity = await db.Patios.FindAsync(id);
    if (entity is null) return Results.NotFound();
...
app.MapGet("/api/zonas", async (AppDbContext db, HttpContext http, [FromQuery] int page = 1, [FromQuery] int pageSize = 10) =>
{
    ...
    Hateoas.AddCollectionLinks(result, http, "/api/zonas");
    return Results.Ok(result);
})
...
app.MapGet("/api/zonas/{id:int}", async (int id, AppDbContext db, HttpContext http) =>
{
    ...
    Hateoas.AddSelf(res, http, $"/api/zonas/{id}");
    Hateoas.AddLink(res, "motos", http, $"/api/motos/zona/{id}", "GET");
    Hateoas.AddLink(res, "patio", http, $"/api/patios/{z.PatioId}", "GET");
    return Results.Ok(res);
})
...
app.MapGet("/api/zonas/patio/{patioId:int}", async (int patioId, AppDbContext db, HttpContext http) =>
{
    ...
    Hateoas.AddCollectionLinks(result, http, $"/api/zonas/patio/{patioId}");
    return Results.Ok(result);
})
...
app.MapPost("/api/zonas", async ([FromBody] ZonaCreateDto body, AppDbContext db, HttpContext http) =>
{
    ...
    var res = new Resource<ZonaDto>(dto);
    Hateoas.AddSelf(res, http, $"/api/zonas/{entity.Id}");
    return Results.Created($"/api/zonas/{entity.Id}", res);
})
.WithSummary("Cria zona")
.WithDescription("Cria uma zona em um pátio.")
.Produces<Resource<ZonaDto>>(StatusCodes.Status201Created);

app.MapPut("/api/zonas/{id:int}", async (int id, [FromBody] ZonaUpdateDto body, AppDbContext db) =>
{
...
app.MapDelete("/api/zonas/{id:int}", async (int id, AppDbContext db) =>
{
    var entity = await db.Zonas.FindAsync(id);
    if (entity is null) return Results.NotFound();
...
app.MapGet("/api/motos", async (AppDbContext db, HttpContext http, [FromQuery] int page = 1, [FromQuery] int pageSize = 10) =>
{
    ...
    Hateoas.AddCollectionLinks(result, http, "/api/motos");
    return Results.Ok(result);
})
...
app.MapGet("/api/motos/{id:int}", async (int id, AppDbContext db, HttpContext http) =>
{
    ...
    Hateoas.AddSelf(res, http, $"/api/motos/{id}");
    if (m.ZonaId != null) Hateoas.AddLink(res, "zona", http, $"/api/zonas/{m.ZonaId}", "GET");
    return Results.Ok(res);
})
...
app.MapGet("/api/motos/zona/{zonaId:int}", async (int zonaId, AppDbContext db, HttpContext http) =>
{
    ...
    Hateoas.AddCollectionLinks(result, http, $"/api/motos/zona/{zonaId}");
    return Results.Ok(result);
})
...
app.MapPost("/api/motos", async ([FromBody] MotoCreateDto body, AppDbContext db, HttpContext http) =>
{
    ...
    var res = new Resource<MotoDto>(dto);
    Hateoas.AddSelf(res, http, $"/api/motos/{entity.Id}");
    return Results.Created($"/api/motos/{entity.Id}", res);
})
...
app.MapPut("/api/motos/{id:int}", async (int id, [FromBody] MotoUpdateDto body, AppDbContext db) =>
{
...
app.MapDelete("/api/motos/{id:int}", async (int id, AppDbContext db) =>
{
    var m = await db.Motos.FindAsync(id);
    if (m is null) return Results.NotFound();
    db.Motos.Remove(m);
...
app.MapPost("/api/motos/{id:int}/mover", async (int id, [FromBody] MotoMoverDto body, AppDbContext db) =>
{
    var m = await db.Motos.FindAsync(id);
    if (m is null) return Results.NotFound();
